...
A01p <- ggplot(A01, aes(x=log2(FSC.H), y=log2(SSC.H))) + 
  geom_point(shape=".", colour="red") + 
  ggtitle("Sample A01 before threeze thaw") + 
  stat_density2d(aes(alpha=..density..), geom="raster", contour = FALSE)



A02p <- ggplot(A01, aes(x=log2(FSC.H), y=log2(SSC.H))) + 
  geom_point(shape=".", colour="red") + 
  ggtitle("Sample A02 before threeze thaw") + 
  stat_density2d(aes(alpha=..density..), geom="raster", contour = FALSE)

A04p <- ggplot(A04, aes(x=log2(FSC.H), y=log2(SSC.H))) + 
  geom_point(shape=".", colour="red") + 
  ggtitle("Sample A04 after threeze thaw") + 
  stat_density2d(aes(alpha=..density..), geom="raster", contour = FALSE)

A05p <- ggplot(A05, aes(x=log2(FSC.H), y=log2(SSC.H))) + 
  geom_point(shape=".", colour="red") + 
  ggtitle("Sample A05 after threeze thaw") + 
  stat_density2d(aes(alpha=..density..), geom="raster", contour = FALSE)

result <- ggarrange(A01p, A02p, A04p, A05p)

ggsave("before_vs_after_freeze.png")
```

```{r}
#Map3 all files from sample 01
filenames=list.files(path ="data/map3/sample01/" ,pattern= '*.csv', full.names = TRUE)
...
```{r}
#Map3 all files from sample 02
filenames=list.files(path ="data/map3/sample02/" ,pattern= '*.csv', full.names = TRUE)
...
```{r}
#Map4 all files from sample 01
filenames=list.files(path ="data/map4/sample01/" ,pattern= '*.csv', full.names = TRUE)
...
```{r}
#Map4 all files from sample 02
filenames=list.files(path ="data/map4/sample02/" ,pattern= '*.csv', full.names = TRUE)
